...
extension ACControlPoint: RequestHandler {
    ...
    public func procedureIDForResponse(_ response: Data, includesSegmentationHeader: Bool = true) -> ProcedureID? {
        // remove segmentation header
        var responseEdited = response
        if includesSegmentationHeader {
            responseEdited = response.subdata(in: response.startIndex+1..<response.count)
        }
